...
from util_3d import getDist, getRay, g_Space, Heuristic, getNearest, isCollide, cost, children, StateSpace, heuristic_fun
...
class Weighted_A_star(object):
    def __init__(self, resolution=0.5):
        ...
        self.OPEN = queue.PriorityQueue()  # store [point,priority]
        ...
        while self.OPEN:  # while xt not reached and open is not empty
            ...
            if N:
                if len(self.CLOSED) % N == 0:
                    break
            if self.ind % 1000 == 0: print('number node expanded = ' + str(len(self.V)))
            self.ind += 1
